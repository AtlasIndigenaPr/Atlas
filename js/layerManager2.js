// js/layerManager.js
import { LAYERS_CONFIG } from './config2.js';
...
export class LayerManager {
    ...
    // -------------------------------------------------------------
    // NOVOS MÉTODOS PARA INTEGRAÇÃO COM A PESQUISA
    // -------------------------------------------------------------
    
    // Retorna a camada do Leaflet para a pesquisa ler os dados
    getCamada(key) {
        return this.leafletLayers[key];
    }

    // Altera o menu do mapa (Radio Buttons) via código
    alternarCamadaAtiva(key) {
        const config = LAYERS_CONFIG[key];
        if (!config) return;

        // Procura os radio buttons nativos do Leaflet na tela
        const inputs = document.querySelectorAll('.leaflet-control-layers-selector');
        
        inputs.forEach(input => {
            // O Leaflet esconde o nome da camada num <span> vizinho ao input
            const spanLabel = input.nextElementSibling;
            if (spanLabel && spanLabel.textContent.trim() === config.title) {
                // Se o botão não estiver marcado, simulamos um clique real nele!
                if (!input.checked) {
                    input.click(); 
                }
            }
    ...
    // 2. Atualiza a barra de pesquisa com o nome da terra indígena clicada
    const props = l.feature.properties;
    const nomeTerra = props.terrai_nom || props.nome || props.nom_ti || "";
    const searchInput = document.getElementById('search-input');
    
    // Atualiza o input se estiver na camada de terras indígenas
    if (searchInput && nomeTerra && (config.id === 'terras_indigenas_fase' || config.id === 'terras_indigenas')) {
        searchInput.value = nomeTerra;
    }

    // 3. Gera o HTML dinâmico baseado nos "fields" do config.js
    const htmlConteudo = this.gerarHtmlPopup(l.feature.properties, config);

    // 4. Abre o Popup do Leaflet na posição do clique
